...
function App() {
  ...
  return (
    <div className="App">
      ...
      <div>
        {soilMoisture ? (
          <p>The current soil moisture is <span style={{color: 'blue', fontWeight: "600"}}>{soilMoisture}</span>%.</p>
        ) : (
          <p>Loading temperature data...</p>
        )}
      </div>
      ...
      <div>
        ...
        <input type="text" value={fanValue} ref={fanInputRef} onChange={e => setFanValue(e.target.value)}/>
        <button onClick={handleFanValueSubmit}>Set</button>
      </div>

      <div>
        <p>Set the value (from 0° to 180°) for yolo servo</p>
        <input type="text" value={servoValue} ref={servoInputRef} onChange={e => setServoValue(e.target.value)}/>
        <button onClick={handleServoValueSubmit}>Set</button>
      </div>

      <div className='my-8'>
        <button onClick={handleTurnOn}  className="rounded-full bg-red-400">Turn on pump</button>
        <button onClick={handleTurnOff} className="rounded-full bg-green-400 ml-2">Turn off pump</button>
      </div>
